...
+ what -> info
  - high-level project docs
    - intro, examples, resources
+ why -- good practice, helps me and strangers alike understand why
...
#+begin_src emacs-lisp :results silent
  (let ((files (list "m" "n" "index" "cv")))
    (dolist (f files)
      (copy-file (concat f ".org") "o/x/org/" 1))
    (copy-file "ox.setup" "o/x/org/" 1)
...
#+begin_src elisp
  (let ((path "o/x/")
        (rhost "/ssh:dmz:dmz/w/")
        (files (list "m" "n" "index" "cv"))
        (packs (list "html" "org" "pdf" "txt" "md")))
    (dolist (f files)
      (copy-file (concat path "html/" f ".html") rhost 1))
    (dolist (p packs)
      (copy-file (concat path p ".tz") (concat rhost "x/") 1)
      (copy-file (concat path p) (concat rhost "x/") 1)))
#+end_src
...
:END:
Once the binary is installed, run =shed init -p= to bootstrap a via
