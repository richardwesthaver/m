...
can be found in [[https://www.gnu.org/software/emacs/manual/html_node/org/Document-Structure.html][the Emacs manual]].

All docs are fully compliant with Org syntax, with the addition of
some special elements that may appear in the body or =section= of any
heading.

- props ::
  All nodes have an optional number of "props" which are denoted as a
  list item that uses =+= followed by the prop symbol and a special
  character. These can appear anywhere in the =section= element of a
  node (not just immediately following properties drawer). They're
  like a shorthand for custom properties.

  #+begin_example
# info props
+ ? ← some long description
+ who/what/when/where/why/how ← some text
+ status ← ACTIVE
+ src ← https://hg.rwest.io/org
...
:END:
#+begin_src shell :exports both :results code replace
shed --help
#+end_src
...
A big inspiration for this project is the [[https://orgmode.org/manual/Library-of-Babel.html][Library of Babel]] - which
allows you to define blocks of code in an Org doc, and call them
remotely. =sn= is in fact just this - a collection of code blocks in
an Org doc. The other collections similarly mirror some set of
features available in Emacs.
...
meta-programming capabilities of Emacs and Org Babel (specifically,
the ability to mix and match functions /from any language/) in a
