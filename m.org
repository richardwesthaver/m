...
| updated | 3 days ago               |
| lines   | 6240 = λ:5792 #:26 _:422 |
| langs   | (Org)                    |
|---------+--------------------------|
#+CALL: meta-make-includes(i='(m.org n.org r.org))
#+RESULTS:
| includes | (m.org n.org r.org) |

#+CALL: meta-make-files-tbl()
#+RESULTS:
| file  | size |          updated |
|-------+------+------------------|
| m.org | 48K  | 2021-11-23-04:45 |
...
** the source                                                      :lp:arch:
:PROPERTIES:
:CUSTOM_ID: meta:source
:END:
The source files are =m.org=, =n.org=, and =r.org= which stand for
=metas=, =notes=, and =rambles= respectively. All other files are
generated /from/ these files.

The source files are written in [[https://orgmode.org/][Org Mode]]. Org syntax is covered [[https://orgmode.org/worg/dev/org-syntax.html][online]]
in detail and high-level info about org-mode document structure can be
found in the [[info:emacs#Top][Emacs manual]].

#+attr_html: :width 300
#+attr_latex: :width 300
[[cdn:img/org-mode-features_display.png]]

Source files are collections where each *top-level heading* is a
different item, category, project, etc. For example, the current file
(=m.org=) contains a heading for each of my software projects.
...
#+SETUPFILE: ox.setup
#+end_src
where %s is the name of the collection and =ox.setup= is a generated
file that doesn't exist on a fresh clone. It includes some boilerplate
headers so the top of our docs look cleaner.

After that it's just a bunch of 'root' headings. They generally have
[[https://orgmode.org/manual/Tags.html][tags]], and a number of [[https://orgmode.org/manual/Properties-and-Columns.html][properties]] such as an =ID= which is used to
generate links. The contents of each heading is vanilla Org syntax but
the structure changes across headings.

All files make use of Org's [[#babel:lp][Literate Programming]] features to provide a
special type of scripting interface - all of the code blocks written
can be accessed by other code blocks. They can be called from anywhere
too, so we can build higher-level /programs/ by chaining them together
in a code block and executing that block.

All lines starting with =#+CALL:= execute a code block which are
wrapped with =#+begin_src %lang ... #+end_src=. There are many
callable blocks, so some care should be taken while casually browsing.

** being terse                                                         :syn:
:PROPERTIES:
:CUSTOM_ID: meta:conventions
:END:
You may have noticed that I use short names in code. Important things
are a single character such as 'm'. Sometimes the syntax I use is a
bit terse, especially in makefiles. Let's talk about that.

First of all it's practical. I type a lot as -- most of us do
nowadays. Short names means less keystrokes, pure and simple. This of
course isn't a good enough reason to use it to the extent you may
find.

My second point is that this is /personal/ software - It's built for
me, and clarity is always a secondary concern. These are programs and
libraries that are optimized for a single user. You should never build
this way for code you want other people to use (unless you're Arthur
Whitney).

Third, I find it more pleasurable to read and write code this
way. Shorter names and less linebreaks means more room for
expression. We have been fooled by the compiler into thinking we need
to document every function, and use clear, verbose variable names so
that the next guy to pick at that function can have a better time
understanding it. It certainly has its place, but it's not the only
way, and sticking to tradition has a way of obstructing innovation. I
am constantly inspired by APLers and how much can be expressed in a
few well-chosen characters as well as the idea of code as
Poetry. Something to be read, written, and enjoyed. These forms
require the reader to put in the work. They don't spoon feed
paragraphs of prose and inline comment every tricky bit. As you piece
it all together though, there is something quite elegant about it.
** scripts
:PROPERTIES:
...
chains together a bunch of code blocks, but otherwise saves me quite a
bit of time. The blocks below are typically executed all at once (in
sequence) and perform the following actions:

- Generate boilerplate - =ox.setup=, =index.org=, and =cv.org=
- Create a publishing directory named =o= in the current directory and
  populate it with our 'meta' files.
- Export meta files from =org= to =html=, =md=, =pdf= and =txt=.
- Collect output formats in separate =tar= archives, compress those
  archives with =zstd=.
- Copy published files to a web-server over =ssh=.
- Delete the publishing directory =o=.
- Generate individual =readme.org= files for all of our code repos
  from headings in =m.org=.
- Add a commit for each repo with an updated =readme.org=
- Push all repos to GitHub mirror.

Each of the actions above are contained in individual code blocks
which can be executed programatically.
...
To start our script we generate the output directory and copy source
files there[fn:1]. This ensures the root project directory isn't polluted on
export.
...
Let's clean some of the junk latex produced by the PDF export, then
...
[fn:1] :: You could also do an initial export to generate pre-parsed
files (with comments and headings tagged with =noexport= removed) in
the output directory instead of copying the raw source. This is great
for docs with sensitive info, but not great for these type of
meta-files where the published document output depend on /execution/
of code blocks contained in the input.
...
| updated | 3 days ago                                        |
| lines   | 3319 = λ:2468 #:415 _:436                         |
| langs   | (Elisp Haskell Makefile Org PowerShell Python Sh) |
|---------+---------------------------------------------------|
#+CALL: meta-make-includes(i='(readme.org makefile lob.org babel.el sc/*))
#+RESULTS:
...
| lob.org    | 15K  | 2021-11-22-23:36 |
| makefile   | 186  | 2021-10-27-02:21 |
| readme.org | 4.6K | 2021-11-23-01:03 |
...
|---------+---------------------------------------|
#+CALL: meta-make-includes(i='(readme.org emacs/ mail/ shell/ term/ vc/ virt/ wm/))
#+RESULTS:
...
#+CALL: meta-make-includes(i='(readme.org makefile Cargo.toml build.rs rustfmt.toml lisp/ src/))
#+RESULTS:
...
|---------+---------------------------|
#+CALL: meta-make-includes(i='(readme.org Cargo.toml rustfmt.toml ./*/))
#+RESULTS:
...
|---------+---------------------------------|
#+CALL: meta-make-includes(i='(readme.org Cargo.toml rustfmt.toml src/ aws/ google/))
#+RESULTS:
