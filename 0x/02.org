...
#+ID: 0x0002
#+FILETAGS: :store:shed:lab:
#+DATE: [2021-04-30]
#+INCLUDE: setup.org
#+STARTUP: overview

#+BEGIN_abstract
0x0002 :: Give Me Data, or Give Me Death!
#+END_abstract

* The Strategy

Our goal is to organize files, data, and documents and it would be
unwise to start adding things manually without having an idea of how
they would arrive at the same point programmatically. What we're after
is a [[https://en.wikipedia.org/wiki/MapReduce][MapReduce]]-type-of system, that works on many types of data.

I definitely do not plan on building such a system right this moment,
but I want to write some code that moves us in that direction.

* How's the Weather?
  Let's do some theorycrafting in +Python+ Rust for a bit.[fn:1]
  
  I think weather forecasting data may be a good way to start
  experimenting, so we'll make a program that fetches forecasts from
...
** Why?
   ...
   - [[file:media/02/zeus.jpg][zeus.jpg]]

** A Word of Caution

   #+ATTR_HTML: :width 200
   [[./media/02/danger.jpeg]]
...
* The Code
  ...
  The Code, which can be found at [[./src/02/]] is split into two separate crates:

  - *[[./src/02/weather][weather]]*
  ...
  files in [[./src/02/data/]].

** Library

*** [[./src/02/weather/src/lib.rs][lib.rs]]
...
*** [[./src/02/weather/src/api.rs][api.rs]]
...
*** [[./src/02/bin/parse_cities.rs][parse_cities.rs]]
...
*** [[./src/02/bin/get_rand.rs][get_rand.rs]]
...
* TL;DR
  ...
  If you want to play with it, download and unpack [[./src/02.tar.zst]],
  then run =make demo= to get the current forecast for Times Square
  printed to stdout.
...
** A Diagram
  #+begin_src napkin :file ./media/02/nws_fch.png :exports results
def seq_diagram(c):
